...
function Library:change_theme(toTheme)
	...
	for color, objects in next, Library.ThemeObjects do
		...
		for _, obj in next, objects do
			...
			if colorAlter < 0 then modifiedColor = Library:darken(themeColor, -1 * colorAlter)
			elseif colorAlter > 0 then modifiedColor = Library:lighten(themeColor, colorAlter) end
...
function Library:object(class, properties)
	...
	function methods:tween(options, callback)
		local options = Library:set_defaults({ Length = 0.2, Style = Enum.EasingStyle.Linear, Direction = Enum.EasingDirection.InOut }, options)
	...
	function methods:tooltip(text)
		local tooltipContainer = methods:object("TextLabel", {
			Theme = { BackgroundColor3 = { "Main", 10 }, TextColor3 = { "WeakText" } },
			TextSize = 16,
			Text = text,
	...
	for property, value in next, properties do
		if customHandlers[property] then customHandlers[property](value)
		else localObject[property] = value end
	end
...
function Library:set_status(txt) self.statusText.Text = "Status | " .. txt end

function Library:set_button_functions(button, default, hover, down) end
...
function Library:create(options)
	...
	closeButton.MouseButton1Click:connect(function()
		...
		core:fade(true)
		wait(0.1)
		core:tween({ Size = UDim2.new() }, function() gui.AbsoluteObject:Destroy() end)
	end)
	local urlBar = core:object("Frame", { Size = UDim2.new(1, -10, 0, 25), Position = UDim2.new(0, 5, 0, 35), Theme = { BackgroundColor3 = "Secondary" } }):round(5)
	...
	local shadowHolder = core:object("Frame", { BackgroundTransparency = 1, Size = UDim2.fromScale(1, 1), ZIndex = 0 })
	...
	local homePage = content:object("Frame", { Size = UDim2.fromScale(1, 1), Centered = true, BackgroundTransparency = 1 })
	local tabs = {}
	-- size handling lol
	--[[
		setmetatable(tabs, {
	...
	local homePageLayout = homePage:object("UIListLayout", { Padding = UDim.new(0, 10), FillDirection = Enum.FillDirection.Vertical, HorizontalAlignment = Enum.HorizontalAlignment.Center })
	local homePagePadding = homePage:object("UIPadding", { PaddingTop = UDim.new(0, 10) })
	local profile = homePage:object("Frame", { AnchorPoint = Vector2.new(0, 0.5), Theme = { BackgroundColor3 = "Secondary" }, Size = UDim2.new(1, -20, 0, 100) }):round(7)
	...
	local quickAccess = homePage:object("Frame", { BackgroundTransparency = 1, Size = UDim2.new(1, -20, 0, 180) })
	...
	local settingsTab = Library.tab(mt, { Name = "Settings", Internal = settingsTabIcon, Icon = "rbxassetid://8559790237" })
	...
	local creditsTab = Library.tab(mt, { Name = "Credits", Internal = creditsTabIcon, Icon = "http://www.roblox.com/asset/?id=8577523456" })
...
function Library:tab(options)
	options = self:set_defaults({ Name = "New Tab", Icon = "rbxassetid://8569322835" }, options)
	...
	if not options.Internal then
		quickAccessButton = self.quickAccess:object("TextButton", { Theme = { BackgroundColor3 = "Secondary" } }):round(5):tooltip(options.Name)
	...
	else quickAccessButton = options.Internal end
	local layout = tab:object("UIListLayout", { Padding = UDim.new(0, 10), HorizontalAlignment = Enum.HorizontalAlignment.Center })
	tab:object("UIPadding", { PaddingTop = UDim.new(0, 10) })
	...
	tabButtonClose.MouseButton1Click:connect(function()
		...
		for _, tab in next, self.Tabs do
			...
			if tab[2].Visible then visible[#visible + 1] = tab end
		end
		local lastTab = visible[#visible]
		if selectedTab == self.homeButton then tab.Visible = false
...
function Library:toggle(options)
	...
	local toggleContainer = self.container:object("TextButton", { Theme = { BackgroundColor3 = "Secondary" }, Size = UDim2.new(1, -20, 0, 52) }):round(7)
	...
	local function toggle()
		toggled = not toggled
		if toggled then offIcon:crossfade(onIcon, 0.1)
		else onIcon:crossfade(offIcon, 0.1) end
	...
	do
		...
		toggleContainer.MouseButton1Click:connect(function() toggle() end)
	end
	self:_resize_tab()
	local methods = {}
...
function Library:dropdown(options)
	...
	local newSize = 0
	local open = false
	local dropdownContainer = self.container:object("TextButton", { Theme = { BackgroundColor3 = "Secondary" }, Size = UDim2.new(1, -20, 0, 52) }):round(7)
	...
	local selectedText = dropdownContainer:object("TextLabel", {
		AnchorPoint = Vector2.new(1, 0),
		Theme = { BackgroundColor3 = { "Secondary", -20 }, TextColor3 = "WeakText" },
	...
	for i, v in next, options.Items do
		if typeof(v) == "table" then items[i] = v
		else items[i] = { tostring(v), v } end
	end
	local toggle
	...
	do
		...
		toggle = function()
			...
			if open then
				...
				dropdownContainer:tween({ Size = UDim2.new(1, -20, 0, 52 + newSize) }, function() self:_resize_tab() end)
				icon:tween{ Rotation = 180, Position = UDim2.new(1, -11, 0, 15) }
			else
				itemContainer:tween{ Size = UDim2.new(1, -10, 0, 0) }
				dropdownContainer:tween({ Size = UDim2.new(1, -20, 0, 52) }, function() self:_resize_tab() end)
		...
		dropdownContainer.MouseButton1Click:connect(function() toggle() end)
	end
	self:_resize_tab()
	local methods = {}
	...
	function methods:Clear()
		...
		dropdownContainer:tween({ Size = UDim2.new(1, -20, 0, 52) }, function()
			for i, v in next, itemContainer.AbsoluteObject:GetChildren() do if v.ClassName == "TextButton" then v:Destroy() end end
		end)
	...
	function methods:AddItems(fitems)
		for i, v in next, fitems do
			if typeof(v) == "table" then items[#items + 1] = v
			else items[#items + 1] = { tostring(v), v } end
		end
		newSize = 25 * #items + 5
		itemContainer:tween{
			Size = not open and UDim2.new(1, -10, 0, 0) or UDim2.new(1, -10, 0, newSize)
		...
		Library._resize_tab{ container = container, layout = layout }
	end
...
function Library:button(options)
	...
	local buttonContainer = self.container:object("TextButton", { Theme = { BackgroundColor3 = "Secondary" }, Size = UDim2.new(1, -20, 0, 52) }):round(7)
	...
	do
		...
		buttonContainer.MouseButton1Click:connect(function() options.Callback() end)
	end
	self:_resize_tab()
	local methods = {}
		...
		buttonContainer.MouseButton1Click:connect(function()
			...
			if options.Style == 1 then
				do
					...
					local _cpShadowHolder = cpHolder:object("Frame", { BackgroundTransparency = 1, Size = UDim2.fromScale(1, 1), ZIndex = 11 })
					...
					do
						...
						hueBar.MouseButton1Down:Connect(function()
							down = true
							while RunService.RenderStepped:Wait() and down do updateHue() end
						end)
						...
						UserInputService.InputEnded:Connect(function(key)
							if key.UserInputType == Enum.UserInputType.MouseButton1 then if down then down = false end end
						end)
					end
					-- END HUE
					-- SAT & VALUE [PICKER]
					do
						...
						pickerArea.MouseButton1Down:Connect(function()
							down = true
							while RunService.RenderStepped:wait() and down do updatePicker() end
						end)
						UserInputService.InputEnded:Connect(function(key)
							if key.UserInputType == Enum.UserInputType.MouseButton1 then if down then down = false end end
						end)
					...
					_cpShadow:tween{ ImageTransparency = 0.6, Length = 0.1 }
					...
					fadeOut = function()
						...
						_colorPickerDraggableStroke:tween({ Transparency = 1, Length = 0.1 }, function()
							...
							task.delay(0.25, function() Library._colorPickerExists = false end)
						end)
					end
				end
			else
				do
					...
					local _holderStroke = holder:object("UIStroke", { Transparency = 1, Theme = { Color = "Tertiary" }, Thickness = 1.6 })
					...
					local pickerArea = holder:object("TextButton", { Text = "", BackgroundTransparency = 1, Size = UDim2.new(0.5, -5, 1, -25) }):round(6)
					local _pickerAreaStroke = pickerArea:object("UIStroke", { Transparency = 1, Theme = { Color = "Tertiary" }, Thickness = 1.6 })
					local color = pickerArea:object("Frame", {
						Size = UDim2.fromScale(1, 1),
						BackgroundColor3 = selectedColor,
						BackgroundTransparency = 1,
					...
					local _hueAreaStroke = hueArea:object("UIStroke", { Transparency = 1, Theme = { Color = "Tertiary" }, Thickness = 1.6 })
					...
					local _infosList = infos:object("UIListLayout", { Padding = UDim.new(0, 4), HorizontalAlignment = Enum.HorizontalAlignment.Center, SortOrder = Enum.SortOrder.Name })
					...
					for _, v in next, darkener.AbsoluteObject:GetDescendants() do pcall(function() v.ZIndex = v.ZIndex + 3 end) end
					...
					do
						updateHue = function()
							...
							hueDraggable:tween{
								Length = 0.05,
								Position = UDim2.new(0, math.clamp(newXPos, 0, hueArea.AbsoluteSize.X), 0.5, 0)
							}
							globalUpdate()
						end
						local down = false
					...
					r:tween{ BackgroundTransparency = 0, TextTransparency = 0, Length = 0.1 }
					g:tween{ BackgroundTransparency = 0, TextTransparency = 0, Length = 0.1 }
					b:tween{ BackgroundTransparency = 0, TextTransparency = 0, Length = 0.1 }
					pickBtn:tween{ BackgroundTransparency = 0, ImageTransparency = 0, Length = 0.1 }
...
function Library:credit(options)
	...
	local creditContainer = (self.creditsContainer or self.container):object("Frame", { Theme = { BackgroundColor3 = "Secondary" }, Size = UDim2.new(1, -20, 0, 52) }):round(7)
	...
	if setclipboard then
		if options.Discord then
			...
			local discord = discordContainer:object("Frame", { Size = UDim2.new(1, -6, 1, -6), Centered = true, BackgroundTransparency = 1 })
			...
			discordContainer.MouseButton1Click:connect(function() setclipboard(options.Discord) end)
		end
		if options.V3rmillion then
			...
			v3rmillionContainer.MouseButton1Click:connect(function() setclipboard(options.V3rmillion) end)
		end
	end
	self._resize_tab{
...
function Library:_theme_selector()
	local themesCount = 0
	for _ in next, Library.Themes do themesCount = themesCount + 1 end
	local themeContainer = self.container:object("Frame", { Theme = { BackgroundColor3 = "Secondary" }, Size = UDim2.new(1, -20, 0, 127) }):round(7)
	...
	local grid = colorThemesContainer:object("UIGridLayout", { CellPadding = UDim2.fromOffset(10, 10), CellSize = UDim2.fromOffset(102, 83), VerticalAlignment = Enum.VerticalAlignment.Center })
	colorThemesContainer:object("UIPadding", { PaddingLeft = UDim.new(0, 10), PaddingTop = UDim.new(0, 5) })
	for themeName, themeColors in next, Library.Themes do
		local count = 0
		for _, color in next, themeColors do if not(type(color) == "boolean") then count = count + 1 end end
		if count >= 5 then
			local theme = colorThemesContainer:object("TextButton", { BackgroundTransparency = 1 })
			local themeColorsContainer = theme:object("Frame", { Size = UDim2.new(1, 0, 1, -20), BackgroundTransparency = 1 }):round(5):stroke("WeakText", 1)
			...
			local colorMain = themeColorsContainer:object("Frame", { Centered = true, Size = UDim2.fromScale(1, 1), BackgroundColor3 = themeColors.Main }):round(4)
			local colorSecondary = colorMain:object("Frame", { Centered = true, Size = UDim2.new(1, -16, 1, -16), BackgroundColor3 = themeColors.Secondary }):round(4)
			colorSecondary:object("UIListLayout", { Padding = UDim.new(0, 5) })
			colorSecondary:object("UIPadding", { PaddingTop = UDim.new(0, 5), PaddingLeft = UDim.new(0, 5) })
			local colorTertiary = colorSecondary:object("Frame", { Size = UDim2.new(1, -20, 0, 9), BackgroundColor3 = themeColors.Tertiary }):round(100)
			local colorStrong = colorSecondary:object("Frame", { Size = UDim2.new(1, -30, 0, 9), BackgroundColor3 = themeColors.StrongText }):round(100)
			local colorTertiary = colorSecondary:object("Frame", { Size = UDim2.new(1, -40, 0, 9), BackgroundColor3 = themeColors.WeakText }):round(100)
			theme.MouseButton1Click:connect(function() Library:change_theme(Library.Themes[themeName]) end)
		end
	end
	self:_resize_tab()
end
...
function Library:keybind(options)
	...
	local keybindContainer = self.container:object("TextButton", { Theme = { BackgroundColor3 = "Secondary" }, Size = UDim2.new(1, -20, 0, 52) }):round(7)
		...
		UserInputService.InputBegan:Connect(function(key)
			if listening then
				if key.UserInputType == Enum.UserInputType.Keyboard then
					if key.KeyCode ~= Enum.KeyCode.Escape then options.Keybind = key.KeyCode end
				...
				if key.KeyCode == options.Keybind then options.Callback() end
			end
		end)
...
function Library:prompt(options)
	...
	local count = 0
	for a, _ in next, options.Buttons do count = count + 1 end
	local darkener = self.core:object("Frame", { BackgroundColor3 = Color3.new(0, 0, 0), BackgroundTransparency = 1, Size = UDim2.fromScale(1, 1) }):round(10)
	...
	local _promptContainerStroke = promptContainer:object("UIStroke", { Theme = { Color = "Tertiary" }, ApplyStrokeMode = Enum.ApplyStrokeMode.Border, Transparency = 1 })
	...
	for text, callback in next, options.Buttons do
		...
		do
			...
			button.MouseButton1Click:connect(function()
				...
				darkener:tween({ BackgroundTransparency = 1, Length = 0.1 }, function()
					darkener.AbsoluteObject:Destroy()
					task.delay(0.25, function() Library._promptExists = false end)
					callback()
				end)
			end)
		end
...
function Library:slider(options)
	...
	local sliderContainer = self.container:object("TextButton", { Theme = { BackgroundColor3 = "Secondary" }, Size = UDim2.new(1, -20, 0, 56) }):round(7)
...
function Library:textbox(options)
	...
	local textboxContainer = self.container:object("TextButton", { Theme = { BackgroundColor3 = "Secondary" }, Size = UDim2.new(1, -20, 0, 52) }):round(7)
	...
	function methods:Set(text) textBox.Text = text end

	return methods
end

return setmetatable(Library, {
	__index = function(_, i) return rawget(Library, i:lower()) end
})
