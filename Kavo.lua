...
function Kavo:ToggleUI()
	if game.CoreGui[LibName].Enabled then game.CoreGui[LibName].Enabled = false
	else game.CoreGui[LibName].Enabled = true end
end
...
function Kavo.CreateLib(kavName, themeList)
	...
	elseif themeList == "Serpent" then themeList = themeStyles.Serpent
		...
		if themeList.SchemeColor == nil then themeList.SchemeColor = Color3.fromRGB(74, 99, 135)
		elseif themeList.Background == nil then themeList.Background = Color3.fromRGB(36, 37, 43)
		elseif themeList.Header == nil then themeList.Header = Color3.fromRGB(28, 29, 34)
		elseif themeList.TextColor == nil then themeList.TextColor = Color3.fromRGB(255, 255, 255)
		elseif themeList.ElementColor == nil then themeList.ElementColor = Color3.fromRGB(32, 32, 38) end
	end
	...
	function Tabs:NewTab(tabName)
		...
		function Sections:NewSection(secName, hidden)
			...
			if hidden then sectionHead.Visible = false
			else sectionHead.Visible = true end
			...
			function Elements:NewButton(bname, tipINf, callback)
				...
				btn.MouseButton1Click:Connect(function()
					if not focusing then
						...
						if btn.AbsoluteSize.X >= btn.AbsoluteSize.Y then size = btn.AbsoluteSize.X * 1.5
						else size = btn.AbsoluteSize.Y * 1.5 end
				...
				btn.MouseLeave:Connect(function()
					if not focusing then
						game.TweenService:Create(btn, TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.In), { BackgroundColor3 = themeList.ElementColor }):Play()
						hovering = false
					end
				end)
				...
				function ButtonFunction:UpdateButton(newTitle) btnInfo.Text = newTitle end
			...
			function Elements:NewTextBox(tname, tTip, callback)
				...
				TextBox.FocusLost:Connect(function(EnterPressed)
					...
					if not EnterPressed then return
					else
			...
			function Elements:NewDropdown(dropname, dropinf, list, callback)
				...
				for i, v in next, list do
					...
					optionSelect.MouseButton1Click:Connect(function()
						if not focusing then
							...
							c.Position = UDim2.new(0, x, 0, y)
							local len, size = 0.35, nil
							if optionSelect.AbsoluteSize.X >= optionSelect.AbsoluteSize.Y then size = optionSelect.AbsoluteSize.X * 1.5
							else size = optionSelect.AbsoluteSize.Y * 1.5 end
							c:TweenSizeAndPosition(UDim2.new(0, size, 0, size), UDim2.new(0.5, -size / 2, 0.5, -size / 2), "Out", "Quad", len, true, nil)
							for i = 1, 10 do
								c.ImageTransparency = c.ImageTransparency + 0.05
								wait(len / 12)
					...
					optionSelect.MouseLeave:Connect(function()
						if not focusing then
							game.TweenService:Create(optionSelect, TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.In), { BackgroundColor3 = themeList.ElementColor }):Play()
							oHover = false
						end
					end)
					coroutine.wrap(function()
				...
				function DropFunction:Refresh(newList)
					newList = newList or {}
					for i, v in next, dropFrame:GetChildren() do if v.Name == "optionSelect" then v:Destroy() end end
			...
			function Elements:NewKeybind(keytext, keyinf, first, callback)
				...
				keybindElement.MouseButton1Click:connect(function(e)
					if not focusing then
						...
						c.Position = UDim2.new(0, x, 0, y)
						local len, size = 0.35, nil
						if keybindElement.AbsoluteSize.X >= keybindElement.AbsoluteSize.Y then size = keybindElement.AbsoluteSize.X * 1.5
						else size = keybindElement.AbsoluteSize.Y * 1.5 end
						c:TweenSizeAndPosition(UDim2.new(0, size, 0, size), UDim2.new(0.5, -size / 2, 0.5, -size / 2), "Out", "Quad", len, true, nil)
						for i = 1, 10 do
							c.ImageTransparency = c.ImageTransparency + 0.05
							wait(len / 12)
				...
				game:GetService"UserInputService".InputBegan:connect(function(current, ok)
					if not ok then if current.KeyCode.Name == oldKey then callback() end end
				end)
				moreInfo.Name = "TipMore"
				moreInfo.Parent = infoContainer
				moreInfo.BackgroundColor3 = Color3.fromRGB(themeList.SchemeColor.r * 255 - 14, themeList.SchemeColor.g * 255 - 17, themeList.SchemeColor.b * 255 - 13)
				...
				keybindElement.MouseLeave:Connect(function()
					if not focusing then
						game.TweenService:Create(keybindElement, TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.In), { BackgroundColor3 = themeList.ElementColor }):Play()
						oHover = false
					end
				end)
			...
			function Elements:NewColorPicker(colText, colInf, defcolor, callback)
				...
				colorElement.MouseLeave:Connect(function()
					if not focusing then
						game.TweenService:Create(colorElement, TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.In), { BackgroundColor3 = themeList.ElementColor }):Play()
						hovering = false
					end
				end)
				if themeList.SchemeColor == Color3.fromRGB(255, 255, 255) then
